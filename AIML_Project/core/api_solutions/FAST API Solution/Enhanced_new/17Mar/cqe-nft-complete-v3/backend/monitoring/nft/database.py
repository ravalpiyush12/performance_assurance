...
class NFTPlatformDatabase:
    ...
    def get_user_lob_access(self, username: str) -> List[Dict]:
        ...
        with self.pool.acquire() as conn:
            ...
            try:
                cursor.execute("""
                    SELECT ID, USERNAME, LOB_NAME, GRANTED_BY, IS_ACTIVE,
                ...
                columns = ['id', 'username', 'lob_name', 'granted_by', 'is_active',
                           'created_date', 'updated_date']
                return [dict(zip(columns, row)) for row in rows]
            except Exception as e:
                logger.error(f"Error getting user LOB access for {username}: {e}")
                return []
            finally:
                cursor.close()
        ...
        with self.pool.acquire() as conn:
            ...
            try:
                cursor.execute("""
                    ...
                    VALUES (:username, :lob_name, :granted_by, 'Y', SYSDATE, SYSDATE)
    ...
    def list_users_with_lob_access(self, lob_name: Optional[str] = None) -> List[Dict]:
        ...
        with self.pool.acquire() as conn:
            ...
            try:
                if lob_name:
                    cursor.execute("""
                        SELECT ID, USERNAME, LOB_NAME, GRANTED_BY, IS_ACTIVE, CREATED_DATE
                        FROM API_NFT_USER_LOB_ACCESS
