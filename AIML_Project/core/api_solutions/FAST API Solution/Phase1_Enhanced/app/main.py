"""
Enhanced Oracle SQL API - Multi-Database with Integrated UI
...
# ========================================
app = FastAPI(
    title="Oracle SQL API - Multi-Database",
    description="Multi-database Oracle SQL API with integrated monitoring UI",
    version="2.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc"
)
...
@app.get("/api/v1/audit/by-database/{database}", tags=["Audit"])
async def get_database_audit_logs(
    ...
    stats = app.state.audit_logger.get_audit_statistics(start_date=start_date)
    ...
    if db_stats["total_events"] > 0 and len(results) > 0:
...
@app.get("/api/v1/audit/failed", tags=["Audit"])
...
@app.post("/api/v1/audit/cleanup", tags=["Audit"])
    ...
    try:
        ...
        if app.state.audit_logger:
            app.state.audit_logger.log_event(
                ...
                database=db_name,
                username=db_config.username,
                sql=request.sql,  # Full SQL for DB, truncated for JSONL file
